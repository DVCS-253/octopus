...
class DVCS_test < Test::Unit::TestCase
	...
	def test_init
		my_workspace.init
		assert_equal(true, File.directory?('.octopus'))
		assert_equal(true, File.directory?('.octopus/revlog'))
		assert_equal(true, File.directory?('.octopus/repo'))
		assert_equal(true, File.directory?('.octopus/communication'))
	end
	...
	#Since clean is to remove all the files under the current folder
	#except for the ones under ./.octopus
	#def test_clean
	#	my_workspace = Workspace.new()
	#	my_workspace.init
